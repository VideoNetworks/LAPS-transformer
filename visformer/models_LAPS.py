...
from visformer.weight_init import to_2tuple, trunc_normal_
import numpy as np

__all__=[
	'visformer_small', 'visformer_tiny', 'visformer_small_hr', 'visformer_small_mr', 'visformer_small_er']
...
class Attention(nn.Module):
	def __init__(self, dim, num_heads=8, head_dim_ratio=1., qkv_bias=False, qk_scale=None,
				 attn_drop=0., proj_drop=0., n_seg=8, fold_div=0, level=0):
		super().__init__()
		self.dim = dim
		...
		self.fold_div = fold_div
		if self.fold_div:
			print("MSHA_E {}/{}".format(dim / self.fold_div, 
				dim))
		self.clip_cnt = 2
		self.level = level
	...
	def shift(self, x):
		...
		bt, y, hw, z = x.shape
		b = bt // t
		fold = z // self.fold_div
		x = x.view(b, t, y, hw, z) # B, T, y, hw, z

		out = torch.zeros_like(x)
		out[:, :-1, :, :, :fold] = x[:, 1:, :, :, :fold] # Left
		out[:, 1:, :, :, fold:2*fold] = x[:, :-1, :, :, fold:2*fold] # Left
		out[:, :, :, :, 2*fold:] = x[:, :, :, :, 2*fold:]
		out = out.view(bt, y, hw, z)		
		return out
	...
	def video_2_pair(self, x):
		# x is in bt, y, hw, z
		t = self.n_seg
		bt, y, hw, z = x.shape
		b = bt // t
		two_index, reverse_index = self.get_t_index()
		x = x.view(b, t, y, hw, z)
		#print("T pair {}".format(two_index))
		x = x[:, list(two_index), :, :, :] # t -> re-order

		tf = 2
		ct = t // tf
		x = x.view(b, tf, ct, y, hw, z)
		x = x.permute(0, 2, 3, 1, 4, 5).contiguous() #b, ct, y, tf, hw, z
		x = x.view(b*ct, y, tf*hw, z)
		return x

	def pair_2_video(self, x):
		# x is in b*ct, y, cc*hw, z)
		t = self.n_seg
		bct, y, tfhw, z = x.shape
		tf = 2
		ct = t // tf
		b = bct // ct
		hw = tfhw // tf
		x = x.view(b, ct, y, tf, hw, z)
		x = x.permute(0, 3, 1, 2, 4, 5).contiguous() # b, tf, ct, y, hw, z
		x = x.view(b, t, y, hw, z)
		two_index, reverse_index = self.get_t_index()
		#print("ReveT pair {}".format(reverse_index))
		x = x[:, list(reverse_index), :, :, :] # t -> blend
		x = x.view(b*t, y, hw, z)

		return x # return bt, y, hw, z
	...
	def forward(self, x):
		...
		if self.level != None:
			#if 1:
			#	q_pre = q.view(B//self.n_seg, self.n_seg, self.num_heads, H*W, self.head_dim)
			q = self.video_2_pair(q)
			k = self.video_2_pair(k)
			v = self.video_2_pair(v)

			#if 1: # debug
			#	q_r = self.pair_2_video(q)
			#	q_r = q_r.view(B//self.n_seg, self.n_seg, self.num_heads, H*W, self.head_dim)
			#	q_new = q.view(B//(self.n_seg), self.n_seg // 2 ,self.num_heads, 2, H*W, self.head_dim)
			#	print("pre {}, \nq {} \nq_after: {}".format(
			#		q_pre[3, :, 1, 1, 10],
			#		q_new[3, :, 1, :, 1, 10],
			#		q_r[3, :, 1, 1, 10]))
		...
		if self.level != None:
			x = self.pair_2_video(x)
			#if 1: # debug
			#	ti, rt = self.get_t_index()
			#	x_new = x.view(B//(self.n_seg), self.n_seg, -1)
			#	x_1 = x_new[:, :-1, :]
			#	x_2 = x_new[:, 1:, :]
			#	sim = torch.nn.functional.cosine_similarity(x_1, x_2, dim=2)
			#	print("t{}\nx {}".format(ti, sim.mean()))
	...
	def __init__(self, dim, num_heads, head_dim_ratio=1., mlp_ratio=4., qkv_bias=False, qk_scale=None,
		...
		if not attn_disabled:
			...
			self.attn = Attention(dim, num_heads=num_heads, head_dim_ratio=head_dim_ratio, qkv_bias=qkv_bias,
								  qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_seg=n_seg, fold_div=fold_div, level=level)
...
class Visformer(nn.Module):
	def __init__(self, img_size=224, patch_size=16, init_channels=32, num_classes=1000, embed_dim=384, depth=12,
				 ...
				 vit_embedding=False, group=8, pool=True, conv_init=False, embedding_norm=None, n_seg=0, fold_div=0, s2_skip_level=[None, None, None, None], s3_skip_level=[None, None, None, None]):
		super().__init__()
		...
		self.n_seg = n_seg
		self.fold_div = fold_div
		self.s2_skip_level = s2_skip_level
		self.s3_skip_level = s3_skip_level
		...
		self.stage2 = nn.ModuleList([
			Block(
				...
				,n_seg=self.n_seg, fold_div=self.fold_div,
				level=self.s2_skip_level[kk],
			)
			for kk, i in enumerate(range(self.stage_num1, self.stage_num1+self.stage_num2))
		])
		...
		self.stage3 = nn.ModuleList([
			Block(
				...
				,n_seg=self.n_seg, fold_div=self.fold_div,
				level=self.s3_skip_level[kk],
			)
			for kk, i in enumerate(range(self.stage_num1+self.stage_num2, depth))
		])
...
def visformer_small_mr(**kwargs):
	model = Visformer(img_size=256, init_channels=32, embed_dim=384, depth=[7,4,4], num_heads=6, mlp_ratio=4., group=8,
					  attn_stage='011', spatial_conv='100', norm_layer=BatchNorm, conv_init=True,
					  embedding_norm=BatchNorm, **kwargs)
	return model

def visformer_small_hr(**kwargs):
	model = Visformer(img_size=320, init_channels=32, embed_dim=384, depth=[7,4,4], num_heads=6, mlp_ratio=4., group=8,
					  attn_stage='011', spatial_conv='100', norm_layer=BatchNorm, conv_init=True,
					  embedding_norm=BatchNorm, **kwargs)
	return model

def visformer_small_er(**kwargs):
	model = Visformer(img_size=360, init_channels=32, embed_dim=384, depth=[7,4,4], num_heads=6, mlp_ratio=4., group=8,
					  attn_stage='011', spatial_conv='100', norm_layer=BatchNorm, conv_init=True,
					  embedding_norm=BatchNorm, **kwargs)
	return model
