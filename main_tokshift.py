...
def main_worker(gpu, ngpus_per_node, args):
	...
	if args.evaluate:
		...
		cfg.TEST.NUM_ENSEMBLE_VIEWS = 10 #10#5
		cfg.TEST.NUM_SPATIAL_CROPS = 3#1
	...
	if args.tune_from:
		...
		if "rest" in args.tune_from:
			preweights = preweights
		elif "MViT" in args.tune_from:
			preweights = preweights["model_state"]
		else:
		...
		set_diff = (keys1 - keys2) 
		print('#### 1Notice: keys that failed to load: {}'.format(set_diff))
		set_diff =  (keys2 - keys1)
		print('#### 2Notice: keys that failed to load: {}'.format(set_diff))
		print('=> New dataset, do not load fc weights')
		preweights = {k: v for k, v in preweights.items() if 'module.base_model.head' not in k}
		preweights = {k: v for k, v in preweights.items() if 'head_dist' not in k}
	...
	# optionally resume2 from a checkpoint
	if args.resume2:
		if os.path.isfile(args.resume2):
			...
			if args.gpu is None:
				...
				# Map model to be loaded to specified single gpu.
				loc = 'cuda:{}'.format(args.gpu)
				checkpoint = torch.load(args.resume2, map_location=loc)
			#if args.gpu is not None:
			#	# best_acc1 may be from a checkpoint from a different GPU
			#	best_acc1 = best_acc1.to(args.gpu)
			k400={k:v for k,v in checkpoint['state_dict'].items() if 'head' not in k}
			model.load_state_dict(k400, strict=False)
			print("=> loaded checkpoint '{}' (epoch {})"
	...
	if args.evaluate:
		...
		acc1, acc5, val_loss = evaluate(test_loader, model, criterion, optimizer, epoch, args, cfg, None)
		print("Evaluate Epoch {}: Acc1 {}, Acc5 {}, Val_loss {}".format(epoch, acc1, acc5, val_loss))
		return
	...
	print(store_name)
	if args.rank == 0:
		wandb.init(project="TokShift-Rep", entity="hzhang57")
		...
		checkpoint_folder = Path('video_checkpoints') / store_name
	...
	for epoch in range(args.start_epoch, total_epoch):
		...
		if tf_writer != None and args.rank == 0:
			...
			wandb.log({"test_loss": val_loss,
						"test_top1_{}x{}".format(cfg.TEST.NUM_SPATIAL_CROPS, cfg.TEST.NUM_ENSEMBLE_VIEWS): acc1,
						"test_top5_{}x{}".format(cfg.TEST.NUM_SPATIAL_CROPS, cfg.TEST.NUM_ENSEMBLE_VIEWS): acc5,
						"train_loss": t_loss,
						"train_top1": t_top1,
						"train_top5": t_top5,
						"learning_rate": t_lr,
	...
	del checkpoint
	acc1, acc5, val_loss = evaluate(test_loader, model, criterion, optimizer, epoch, args, cfg, tf_writer)
	wandb.log({"best_{}x{}_test_loss".format(cfg.TEST.NUM_SPATIAL_CROPS, cfg.TEST.NUM_ENSEMBLE_VIEWS): val_loss,
				"best_{}x{}_test_top1".format(cfg.TEST.NUM_SPATIAL_CROPS, cfg.TEST.NUM_ENSEMBLE_VIEWS): acc1,
				"best_{}x{}_test_top5".format(cfg.TEST.NUM_SPATIAL_CROPS, cfg.TEST.NUM_ENSEMBLE_VIEWS): acc5,
			})
...
def evaluate(test_loader, model, criterion, optimizer, epoch, args, cfg, tf_writer):
	...
	print(test_meter.stats["top1_acc"])
	return float(test_meter.stats["top1_acc"]), float(test_meter.stats["top5_acc"]), losses.avg
