...
class Block(nn.Module):
	...
	def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
				 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, n_seg=8, fold_div=8):
		super().__init__()
		...
		print("Shift Token COnfigures n_seg {}, fold_div {}".format(self.n_seg, self.fold_div))
...
class VisionTransformer(nn.Module):
	...
	def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
				 ...
				 act_layer=None, weight_init='', n_seg=8, fold_div=8):
		"""
		Args:
		...
		self.blocks = nn.Sequential(*[
			Block(
				...
				drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, n_seg=self.n_seg, fold_div=self.fold_div)
...
@register_model
def vit_base_patch16_224_in21k(pretrained=False, n_seg=8, fold_div=8, **kwargs):
	""" ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
	ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
	"""
	model_kwargs = dict(
		patch_size=16, embed_dim=768, depth=12, num_heads=12, representation_size=768, n_seg=n_seg, fold_div=fold_div, **kwargs)
