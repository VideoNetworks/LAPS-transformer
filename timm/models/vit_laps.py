...
class Attention(nn.Module):
	def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., n_seg=9, fold_div=0, level=0):
		super().__init__()
...
class Block(nn.Module):
	...
	def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
				 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, n_seg=0, fold_div=0, level=0):
		super().__init__()
		self.norm1 = norm_layer(dim)
		self.attn = Attention(
			dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_seg=n_seg, fold_div=fold_div, level=level)
...
class VisionTransformer(nn.Module):
	...
	def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
				 ...
				 act_layer=None, weight_init='', n_seg=0, fold_div=0, s2_skip_level=[None, None, None, None]):
		"""
		Args:
		...
		self.blocks = nn.Sequential(*[
			Block(
				...
				drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, n_seg=self.n_seg, fold_div=self.fold_div, level=self.s2_skip_level[kk%4])
			for kk, i in enumerate(range(depth))])
