...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous() #  B, N, Y, Z-> B, Y, N, Z
        if self.sr_ratio > 1:
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            ...
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, apply_transform=apply_transform, n_seg=n_seg, fold_div=fold_div, level=level)
...
class ResT(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 norm_layer=nn.LayerNorm, apply_transform=False, n_seg=8, fold_div=0, s2_skip_level=[None, None]):
        super().__init__()
        ...
        self.stage1 = nn.ModuleList([
            Block(embed_dims[0], num_heads[0], mlp_ratios[0], qkv_bias, qk_scale, drop_rate, attn_drop_rate,
                  ...
                  n_seg=n_seg, fold_div=fold_div, level=s2_skip_level[i])
