...
class Attention(nn.Module):
    ...
    def video_2_clip(self, x):
        ...
        ct = t // cc
        #x = x.view(b, t, y, hw, z)
        x = x.view(b, cc, ct, y, hw, z)
        x = x.permute(0, 2, 3, 1, 4, 5).contiguous() #b, ct, y, cc, hw, z
        x = x.view(b*ct, y, cc*hw, z)
        return x

    def clip_2_video(self, x):
        # x is in b*ct, y, cc*hw, z)
        t = self.n_seg
        bct, y, cchw, z = x.shape
        cc = self.clip_cnt
        ct = t // cc
        b = bct // ct
        hw = cchw // cc
        x = x.view(b, ct, y, cc, hw, z)
        x = x.permute(0, 3, 1, 2, 4, 5).contiguous() # b, cc, ct, y, hw, z
        x = x.view(b*cc*ct, y, hw, z)
    ...
    def forward(self, x, H, W):
        ...
        k, v = kv[0], kv[1]
        if self.level != None:
            q = self.video_2_clip(q)
            k = self.video_2_clip(k)
            v = self.video_2_clip(v)
